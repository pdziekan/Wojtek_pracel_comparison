...
c        call integxz(thn,scr2,nx,nz)
      ...
      pi=4.*atan(1.)
       ...
       do i=1,nx
         ...
         del=1.0
         coe=pi/2. * (rad-200.)/100.
         coe=cos(coe)*cos(coe)
         if(rad.ge.200.) del=0.2+0.8*coe
         if(rad.ge.300.) del=0.

cc for rh=0.4 NOTE: RH=0.8 in prof_init, limt below...
        theta(i,k)=th_e(k)
        relhum=0.2+del*0.8
...
c       scr1(i,k)=gg*(epsb*(qv(i,k)-qv_e(k))-qc(i,k)-qr(i,k))
       enddo
       enddo
...
c      call set(.1,.9,.2,.6,0.,10.,0.,2.,1)           
c      call labmod('(i4)','(f5.0)',4,4,2,2,20,20,0)     
c      call periml(2,5,2,5)                           
c      call dashdc('$$$$$$$$$$$$$$$$$$$$',10,12)      
c      call curved(cntti,cntrm,ntime)                  
c      call set(0.,1.,0.,1.,0.,1.,0.,1.,1)
c      CALL plchhq(.5,0.13, 'time (min)', 0.016,0.,0)
c      CALL plchhq(.03,0.4, 'qc center of mass (km)', 0.016,90.,0)
c      call frame                                   
...
c      print 205, eer,eem,niter,nitav
...
c      call cpsetc('ILT',' ')
c      call cpseti('LLP',0)
c      call cpcnrc(pl,nx,nx,nz,290.,590.,2.,-1,-1,-682)
c      call set(0.,1.,0.,1.,0.,1.,0.,1.,1)
...
c      qv(i,k)=qv(i,k)-delta
c      qc(i,k)=qc(i,k)+delta
...
cc
      ...
      fth(i,k)=0;
c      fqr(i,k)=devp+dcol
c      fqc(i,k)=fqc(i,k)-dcol
c      fqv(i,k)=fqv(i,k)-devp
c      fth(i,k)=fth(i,k)+c*devp*thetme
...
cc input sounding, two layers:
      st1= 1.3E-05 ! assumed stability:
      st2= 3.0E-05 ! assumed stability:
      cap=rg/cp
      capi=1./cap
...
cc 0-nz/2
      coe_l=comb(tm_e(1),tdn,tup,0.,1.)   ! liquid contribution
      tt=tm_e(1)
      delt=(tt-tt0)/(tt*tt0)
      esw=ee0*exp(d * delt)
      esi=ee0*exp(b * delt)
      qvsw=a * esw /(pres*1.e2-esw)
      qvsi=a * esi /(pres*1.e2-esi)
      qvs=coe_l*qvsw + (1.-coe_l)*qvsi
      qv_e(1)=relhum*qvs
      ...
      do k=2,nz/2
ccc
       relhum=.75 + .20*height(k)/height(nz/2)
ccc
      zz(k)=height(k)
        exs=exp(-st1*zz(k))
        th0(k)=th0(1)/exs
        ...
        tm_e(k)=tm_e(1)/exs*(1.-cs*(1.-exs))
        pres=1.e3*(th_e(k)/tm_e(k))**(-cp/rg)
...
c        ux_e(k)=ux_e(1)
        ux_e(k)=ux_e(1) + height(k)*2./5.e3
        uy_e(k)=uy_e(1)
c       print*,'  k,z,p.th0: ',k,zz(k),presnl,th0(k)
c       print*,'rh0,the,tme,qv: ',rho0(k),th_e(k),tm_e(k),qv_e(k)
      end do
 
cc nz/2 - nz
      z00=height(nz/2)
      cs=gg/(cp*tm_e(nz/2)*st2)
ccc
       relhum=.95
ccc
      do k=nz/2+1,nz
      zz(k)=height(k) - z00
        ...
        th0(k)=th0(nz/2)/exs
        th_e(k)=th0(k)
        rho0(k)=rho0(nz/2)*exs*(1.-cs*(1.-exs))**(capi-1.)
        tm_e(k)=tm_e(nz/2)/exs*(1.-cs*(1.-exs))
        pres=1.e3*(th_e(k)/tm_e(k))**(-cp/rg)
...
cc surface values for dry profiles:
      ...
      th_e(1)=th0(1)*(1.+a*qv_e(1))
         ...
         k=1
c         print*,'z,the,tme,qve: ',zz,th_e(k),tm_e(k),qv_e(k)
...
c       print*,'checking stability: ',stab,st
cc compute reference state vertical profiles
      cap=rg/cp
      capi=1./cap
      cs=gg/(cp*tm_e(1)*st)
      zz=0.
c      print*,'z,th0,rho0: ',zz,th0(1),rho0(1)
      do k=2,nz
      zz=(k-1)*dz
      exs=exp(-st*zz)
      th0(k)=th0(1)/exs
      rho0(k)=rho0(1)*exs*(1.-cs*(1.-exs))**(capi-1.)
c      print*,'z,th0,rho0: ',zz,th0(k),rho0(k)
      enddo
...
cc surface data:
      ...
      subroutine prof_init_1layer  ! dupa
      parameter(nx=181,nz=121)
      common/grid/ time,dt,dx,dz,dti,dxi,dzi
      common /strtch/ height(nz),gac(nz)
...
cc surface data:
      tm_e(1)=283.
      pres=0.850e3
      rho0(1)=pres*100./rg/tm_e(1)
      relhum=.2
      th_e(1)=tm_e(1)*(1.e3/pres)**(rg/cp)
      th0(1) =th_e(1)
      pres0=pres
      ...
      zz(k)=height(k)
        ...
        rho0(k)=rho0(1)*exs*(1.-cs*(1.-exs))**(capi-1.)
        tm_e(k)=tm_e(1)/exs*(1.-cs*(1.-exs))
        pres=pres0*(th_e(k)/tm_e(k))**(-cp/rg)
       ...
       qv_e(k)=relhum*qvs
        ...
        ux_e(k)=ux_e(1)
c        ux_e(k)=ux_e(1) + height(k)*2./5.e3
        uy_e(k)=uy_e(1)
c       print*,'  k,z,p.th0: ',k,zz(k),pres,th0(k)
c       print*,'rh0,the,tme,qv: ',rho0(k),th_e(k),tm_e(k),qv_e(k)
      end do
...
cc check consistency (int gac ddzeta = H)
      ...
      sum1=sum1+gac(i)*dz
      enddo
c      print*,'int Jacobian before adj: ',sum1
...
cc check:
      ...
      sum1=sum1+gac(i)*dz
      enddo
c      print*,'int Jacobian after adj: ',sum1
