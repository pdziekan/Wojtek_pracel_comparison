...
ccc temperature; convert qv to SI units:
          ...
          do k=1,npin
c          print*,'z,p,th,t,qv: ',zin(k),press(k),theta(k),temp(k),vap(k)
          enddo
...
c         print*,' ***** profiles: *****'
         do k=1,nz
c         print 102, z(k),te(k),pre(k),the(k),qve(k)
...
cC    first guess
c       tempk=t(k)
cc if the next line is open, the first guess is delta=0 each time
cC however, it makes more sense to leave delta from previous level
cc to be the first guess, so the line is commented out
c       delta=0.
cC    iterate:
c                   do it=1,10
cCcccccccccccccccccccccccccccccc
cCcc below is printout to see convergence:
cC          if(k.gt.9.and.k.lt.21) then
cC            print*,'** k,it,delta: ',k,it,delta
cC          endif
cCccccccccccccccccccccccccccccccccc
c         es=esat(tempk)
c         qvs=0.621*es/(pre(k)-es) 
c          top=qvs-qv(k)+delta
c          bottom=.621*pre(k)/(pre(k)-es)**2  * hlat*es/(rv*tempk**2) 
c     1     * hlat/cp   +  1. 
c
c         delta = delta - top/bottom
c         tempk=t(k)+hlat/cp*delta
c                 enddo
c
cC final adjustement:
c          delta = amax1(delta,-qc(k)) ! <-- limit for evaporation
c          th(k)=th(k) +  hlat/cp * the(k)/te(k) *delta
c          qv(k)=qv(k)-delta
c          qc(k)=qc(k)+delta
c         
c          t(k)=th(k)*te(k)/the(k)  !<-- convert pot. temp into temp
...
c         enddo
         ...
         do k=1,nz
         print 104, z(k),qc(k),th(k),rho(k)
104      format(1x,4e12.4,'    z,qc,th,rho: ')
         enddo
